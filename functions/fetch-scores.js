...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const { league, season, matchday } = event.queryStringParameters || {};
    
    console.log('Parameters:', { league, season, matchday });
    ...
    if (!league || !season || !matchday) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error: 'Missing required parameters: league, season, matchday'
        })
      };
    }
    ...
    const transformedFixtures = data.fixtures.map(fixture => {
