...
export class FootballWebPagesAPI {
    ...
    loadConfiguration() {
        ...
        if (typeof window !== 'undefined') {
            ...
            if (window.FOOTBALL_WEBPAGES_CONFIG && window.FOOTBALL_WEBPAGES_CONFIG.RAPIDAPI_KEY) {
                ...
                console.log('✅ Football Web Pages API configuration loaded from window.FOOTBALL_WEBPAGES_CONFIG');
                console.log('API Key available:', !!this.config.RAPIDAPI_KEY);
                return true;
            }
            
            // Try to access from ENV_CONFIG (new environment configuration)
            if (window.ENV_CONFIG && window.ENV_CONFIG.RAPIDAPI_KEY) {
                this.config = {
                ...
                console.log('✅ Football Web Pages API configuration loaded from window.ENV_CONFIG');
                console.log('API Key available:', !!this.config.RAPIDAPI_KEY);
                return true;
            }
            
            // Try to access from global RAPIDAPI_KEY (where your API key actually is)
            if (window.RAPIDAPI_KEY) {
                ...
                console.log('✅ Football Web Pages API configuration loaded from window.RAPIDAPI_KEY');
                console.log('API Key available:', !!this.config.RAPIDAPI_KEY);
                return true;
            }
            
            // Try to access from footballWebPagesAPI if it exists
            if (window.footballWebPagesAPI && window.footballWebPagesAPI.config && window.footballWebPagesAPI.config.RAPIDAPI_KEY) {
                this.config = window.footballWebPagesAPI.config;
                console.log('✅ Football Web Pages API configuration loaded from window.footballWebPagesAPI.config');
                console.log('API Key available:', !!this.config.RAPIDAPI_KEY);
                return true;
            }
            
            // Try to access from apiManager if it exists
            if (window.apiManager && window.apiManager.footballWebPagesAPI && window.apiManager.footballWebPagesAPI.config && window.apiManager.footballWebPagesAPI.config.RAPIDAPI_KEY) {
                this.config = window.apiManager.footballWebPagesAPI.config;
                console.log('✅ Football Web Pages API configuration loaded from window.apiManager.footballWebPagesAPI.config');
                console.log('API Key available:', !!this.config.RAPIDAPI_KEY);
                return true;
            }
        }
        ...
        console.log('🔍 Available API configuration objects:', {
            FOOTBALL_WEBPAGES_CONFIG: !!window.FOOTBALL_WEBPAGES_CONFIG,
    ...
    retryLoadConfiguration() {
        ...
        const attemptLoad = () => {
            ...
            if (typeof window !== 'undefined') {
                ...
                if (window.FOOTBALL_WEBPAGES_CONFIG && window.FOOTBALL_WEBPAGES_CONFIG.RAPIDAPI_KEY) {
                    this.config = window.FOOTBALL_WEBPAGES_CONFIG;
                    console.log('✅ Football Web Pages API configuration loaded from window.FOOTBALL_WEBPAGES_CONFIG on retry attempt', attempts);
                    return;
                }
                ...
                if (window.ENV_CONFIG && window.ENV_CONFIG.RAPIDAPI_KEY) {
                    ...
                    console.log('✅ Football Web Pages API configuration loaded from window.ENV_CONFIG on retry attempt', attempts);
                    return;
                }
                ...
                if (window.RAPIDAPI_KEY) {
                    ...
                    console.log('✅ Football Web Pages API configuration loaded from window.RAPIDAPI_KEY on retry attempt', attempts);
                    return;
                }
                ...
                if (window.footballWebPagesAPI && window.footballWebPagesAPI.config && window.footballWebPagesAPI.config.RAPIDAPI_KEY) {
                    this.config = window.footballWebPagesAPI.config;
                    console.log('✅ Football Web Pages API configuration loaded from window.footballWebPagesAPI.config on retry attempt', attempts);
                    return;
                }
                ...
                if (window.apiManager && window.apiManager.footballWebPagesAPI && window.apiManager.footballWebPagesAPI.config && window.apiManager.footballWebPagesAPI.config.RAPIDAPI_KEY) {
                    this.config = window.apiManager.footballWebPagesAPI.config;
                    console.log('✅ Football Web Pages API configuration loaded from window.apiManager.footballWebPagesAPI.config on retry attempt', attempts);
                    return;
                }
            }
            ...
            if (attempts < maxAttempts) {
                ...
                setTimeout(() => {
                    if (typeof window !== 'undefined') {
                        if (window.FOOTBALL_WEBPAGES_CONFIG && window.FOOTBALL_WEBPAGES_CONFIG.RAPIDAPI_KEY) {
                            this.config = window.FOOTBALL_WEBPAGES_CONFIG;
                            console.log('✅ Football Web Pages API configuration loaded from window.FOOTBALL_WEBPAGES_CONFIG on final attempt');
                            ...
                            console.log('✅ Football Web Pages API configuration loaded from window.ENV_CONFIG on final attempt');
                            ...
                            console.log('✅ Football Web Pages API configuration loaded from window.RAPIDAPI_KEY on final attempt');
                        } else if (window.footballWebPagesAPI && window.footballWebPagesAPI.config && window.footballWebPagesAPI.config.RAPIDAPI_KEY) {
                            this.config = window.footballWebPagesAPI.config;
                            console.log('✅ Football Web Pages API configuration loaded from window.footballWebPagesAPI.config on final attempt');
                            ...
                            this.config = window.apiManager.footballWebPagesAPI.config;
                            console.log('✅ Football Web Pages API configuration loaded from window.apiManager.footballWebPagesAPI.config on final attempt');
                        }
    ...
    async importSelectedFixtures() {
        ...
        // Get the selected game week from the import selector
        const gameWeekSelect = document.querySelector('#import-gameweek-select');
        if (!gameWeekSelect) {
            console.error('❌ Import gameweek selector not found');
            alert('Import gameweek selector not found. Please refresh the page.');
            return;
        }
        ...
        // Get the active edition from the admin panel
        const activeEditionSelect = document.querySelector('#quick-edition-selector');
        if (!activeEditionSelect) {
            console.error('❌ Active edition selector not found');
            alert('Active edition selector not found. Please refresh the page.');
            return;
        }
        
        const activeEdition = activeEditionSelect.value;
        if (!activeEdition) {
            console.error('❌ No active edition selected');
            alert('Please select an active edition first.');
            return;
        }
        
        console.log(`🔍 Import settings - Edition: ${activeEdition}, Game Week: ${gameWeek}`);
        
        // Show loading status
        const statusElement = document.querySelector('#import-status');
        if (statusElement) {
            statusElement.textContent = `🔄 Importing ${selectedFixtures.length} fixtures to Edition ${activeEdition}, Game Week ${gameWeek}...`;
            statusElement.className = 'status-message loading';
        ...
        console.log(`📥 Importing ${selectedFixtures.length} fixtures to Edition ${activeEdition}, Game Week ${gameWeek}:`, selectedFixtures);
        ...
        try {
            ...
            const fixturesToSave = selectedFixtures.map(fixture => {
                ...
                console.log(`🔄 Converting fixture: ${fixture['home-team']?.name || 'Unknown'} vs ${fixture['away-team']?.name || 'Unknown'} -> ${convertedFixture.homeTeam} vs ${convertedFixture.awayTeam}`);
                return convertedFixture;
            });
            
            console.log(`📋 Converted ${fixturesToSave.length} fixtures:`, fixturesToSave);
            
            // Use the same format as the fixtures manager
            const gameweekKey = gameWeek === 'tiebreak' ? 'gwtiebreak' : `gw${gameWeek}`;
            const editionGameweekKey = `edition${activeEdition}_${gameweekKey}`;
            
            console.log(`🔑 Database key: ${editionGameweekKey}`);
            
            // Save to Firestore
            if (this.db) {
                console.log(`💾 Saving to Firestore: collection='fixtures', document='${editionGameweekKey}'`);
                ...
                console.log('🔍 Available database references:', {
                    thisDb: !!this.db,
                    windowDb: !!window.db,
                    appDb: !!(window.app && window.app.db),
                    firebaseDb: !!window.firebase
                });
                throw new Error('Database not available');
            }
            
            console.log(`📥 Successfully saved ${fixturesToSave.length} fixtures to Edition ${activeEdition}, Game Week ${gameWeek}`);
            alert(`Successfully imported ${fixturesToSave.length} fixtures to Edition ${activeEdition}, Game Week ${gameWeek}!`);
            
            // Refresh the fixture management display if it exists
            if (window.app && window.app.fixturesManager) {
                console.log('🔄 Refreshing fixture management display...');
                window.app.fixturesManager.loadFixturesForGameweek();
            } else if (typeof window.loadFixturesForGameweek === 'function') {
                console.log('🔄 Refreshing fixture management display via global function...');
                window.loadFixturesForGameweek();
            } else {
                console.log('⚠️ Fixture management refresh function not found');
                
                // Try to find and click the refresh button in Fixture Management
                const refreshBtn = document.querySelector('#refresh-fixtures-btn, .refresh-fixtures-btn, [onclick*="loadFixturesForGameweek"]');
                if (refreshBtn) {
                    console.log('🔄 Found refresh button, clicking it...');
                    refreshBtn.click();
                } else {
                    console.log('⚠️ No refresh button found, fixtures may need manual refresh');
                }
            }
            ...
            const statusElement = document.querySelector('#import-status');
            if (statusElement) {
                statusElement.textContent = `Successfully imported ${fixturesToSave.length} fixtures to Edition ${activeEdition}, Game Week ${gameWeek}`;
                statusElement.className = 'status-message success';
                statusElement.style.display = 'block';
            ...
            if (statusElement) {
                ...
                // Hide the status message after 10 seconds for errors
                setTimeout(() => {
                    statusElement.style.display = 'none';
                }, 10000);
            }
